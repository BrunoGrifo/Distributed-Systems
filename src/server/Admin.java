...
public class Admin {
	...
	public static void main(String args[]) throws RemoteException {
		...
		while (true) {
					...
					if (!rmi.return_facultys().isEmpty()) {
						...
						if (option != 0) {
							...
							if (!chosen_faculty.departments.isEmpty()) {
								...
								out.println(rmi.print_department_faculty(chosen_faculty));
								out.print("0-Exit");
						...
						while (option != 0) {
							try {
								...
								if (date.after(write_date_format)) {
									System.out.println("Date is after today date expiration date invalide");
									option = 1;
								}
			...
			case 7:
				...
				ArrayList<Faculty> facultys = rmi.return_facultys();// por os arrays dentro dos while
				j = 1;
				option = 1;
				if (facultys.isEmpty()) {
					...
					while (option != 0 && j != 0) {
						...
						for (i = 0; i < 2; i++) {
							...
							for (Faculty x : facultys) {
								check = 0;
								for (Pessoa k : rmi.return_pessoa()) { // recursivo? vale a pena?
									if (k.faculdade != null) {
					...
					while (option != 0 && j != 0) {
						...
						for (i = 0; i < 2; i++) {
							...
							for (Department x : deps) {
								...
								for (Mesas d : rmi.return_mesas()) {// recursivo
			...
			case 9:
				...
				if (eleicoes.isEmpty()) {
					...
					while (option != 0 && eleicoes.size() != 0) {
						...
						if (option != 0) {
							output = rmi.remove_election(eleicoes.get(option - 1).titulo);
							out.print(output);
						}
						eleicoes = rmi.return_elections();
					}
			...
			case 11:
						...
						while (option != 0) {
								...
								if (rmi.return_facultys().isEmpty()) {
									...
									if (option == 0) {
										...
										if (option == 0) {
											...
											select_person.departamento = new_faculty.departments.get(option - 1);
											output = rmi.update_person(select_person);
											out.print(output);
											if (!output.equals("Person updated\n")) {
												option = 0;
							...
							case 4:
								if (select_person.faculdade != null) {
									...
									out.print("Impossible action beacuase the person does not have a faculty\n");
								}
								break;
							case 5:
							...
							case 6:
								...
								break;
							/*
							 * case 7: number_cc_string = ""; do { do { out.print("Current cc number: " +
							 * select_person.numero_cc + "\nNumber of cc(8 digits):"); try {
							 * number_cc_string = inputS.nextLine(); } catch (InputMismatchException e) {
							 * out.println("Invalid option"); inputI.next(); } } while
							 * (!checkNumbers(number_cc_string) || number_cc_string.length() != 8);
							 * number_cc = Integer.parseInt(number_cc_string); } while
							 * (!checkCCnumber(number_cc, rmi.return_pessoa()) || number_cc !=
							 * select_person.numero_cc); out.print("cc number updated\n"); break;
							 */
							case 7:
								while (option != 0) {
									try {
				...
				break;// e se uma pessoa for adiciona na momento que outra foi apagada?WARNING
			case 12:
				...
				break;
			/*
			 * case 13: String new_name; if (rmi.return_departments().isEmpty()) {
			 * out.print("Theres is no departments avaible\n"); } else {
			 * out.print("Chose department you want to change the name:\n");
			 * out.print(rmi.print_departments()); out.print("0-Exit"); option =
			 * testOption(0, rmi.return_departments().size()); if (option != 0) {
			 * out.print("New name of department:"); do { new_name = inputS.nextLine(); }
			 * while (!checkStringWithNumbers(new_name));
			 * rmi.return_departments().get(option - 1).name = new_name; } } break;
			 */
			case 13:
				int option2;
				String write;
				...
				if (list_elections.isEmpty()) {
					...
					out.print("Elections:\n");
					out.print(rmi.print_election2());
					out.print("\n0-Exit\n");
					out.print("Chose:");
					option = testOption(0, list_elections.size());
					if (option != 0) {
						Eleicoes election = list_elections.get(option - 1);
						while (option != 0) {
							out.print("Select the field you want to change:\n" + "1-Change type\n"
							 ...
							 * case 4:
							 * 
							 * title = inputS.nextLine(); do { out.print("Current title of election: " +
							 * election.titulo + "\nNew title of election:"); write = inputS.nextLine();
							 * while (rmi.search_election(title) == false) {
							 * out.print("Name already in use chose a new one:"); do { name =
							 * inputS.nextLine(); } while (!checkStringWithNumbers(name)); }
							 * 
							 * } while (!checkStringWithNumbers(title));
							 * 
							 * election.titulo = write; out.print("Title updated\n"); break;
							 */
							case 4:
							...
							case 6:
								...
								if (option2 == 1) {
									...
									if (rmi.return_mesas().isEmpty()) {
										...
										option2 = 1;
										while (option2 != 0 && i != 0) {
											for (j = 0; j < 2; j++) {// algures um erro aqui
												i = 0;
												if (j == 0) {
													out.print("Table avaible do be added to the election:\n");
												}
						...
						if (option != 0) {
							if (option == 1) {
								...
								for (j = 0; j < 2; j++) {
									...
									for (Department x : rmi.return_departments()) { // mudar isto para nao ser tao
																					// recursivo?
										check = 0;
										for (Mesas k : rmi.return_mesas()) {
											if (k.departamento != null) {
	...
	public static void delete_lists(String type, HashMap<Lista_candidata, Integer> list_lists) {
        ...
        while (!delete.equals("done") && i != 0) {
            ...
            for (int j = 0; j < 2; j++) {
                ...
                if (j == 1 && i != 0) {
                    if (!delete.equals("done")) {//esta mal aqui
	...
	public static void assing_departments_to_facultys(Faculty faculty_chosen) throws RemoteException {
		...
		for (int j = 0; j < 2; j++) {
			...
			for (Department k : rmi.return_departments()) { // recursividade aqui?
				check = 0;
		...
		while (option != 0) {
			...
			for (int j = 0; j < 2; j++) {
				...
				for (Department x : faculty_chosen.departments) {
					check = 0;
					for (Pessoa k : rmi.return_pessoa()) { // recursividade?
						if (k.departamento != null) {
							if (k.departamento.name.equals(x.name)) {
								check = 1;
							}
